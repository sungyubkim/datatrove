...
This file provides guidance to autonomous coding agents (GPT, Claude, Gemini, etc.) when working with the DataTrove repository. It mirrors the expectations in `CLAUDE.md` but broadens them for any agent automation.

## Agent Workflow Notes

- Start every task by reviewing `README.md`, `CLAUDE.md`, and open issues/PR context so you understand current priorities.
- Prefer `uv` for installing dependencies, running scripts, and executing tests to ensure consistent virtual environments.
- When editing files, favor incremental commits, note any assumptions in your response, and surface uncertainties early.
- Run the most specific test or lint command that exercises the code you touched before handing work back to humans.
- If you cannot complete a task, clearly describe partial progress, blockers, and proposed next steps.
